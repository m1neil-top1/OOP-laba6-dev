...
class Lowyer(Bank_worker):
    ...
    def __init__(self,
                 ...
                 document="",
                 salary=10000) -> None:
        super().__init__(phone_numbe, nama, document, "Юрист консультант",
                         salary)
    ...
    def treaty(self, client=Client()):
        ...
        while user_signature.lower() != "y" and user_signature.lower() != "n":
            user_signature = input(
                "Вы согласны со всеми условиями договора? y/n: ")
        ...
        if user_signature.lower() == "y":
            ...
            while user_signature.lower() != "y" and user_signature.lower(
            ) != "n":
                user_signature = input(
                    "Вы отказываетесь от офрмления кредита? y/n: ")
                if user_signature.lower() != "y" and user_signature.lower(
                ) != "n":
                    print("Не корректынй ответ! Повторите попытку!")
            if user_signature.lower() == "y":
                client.set_credit(0)
                client.set_credit_time(0)
                client.get_sum_use_credit(0)
