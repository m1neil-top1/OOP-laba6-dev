...
class Manager(Bank_worker):
    def __init__(self, phone_numbe="None", nama="None", document=str(), salary=float()) -> None:
    ...
    def check_client_for_get_credit(self, client=Client()):
        if (
            ...
            or client.get_work_place() == None
            or client.get_work_position() == None
            or client.get_salary() == 0.0
            or client.get_age() == 0
        ):
            print(
        ...
        if client.get_has_credit():
            print("Вы сможите оформить кредит повторно когда оплате уже существующих.")
            return False
        elif client.get_age() < 18:
            print("Оформить кредит можно только с 18 лет.")
            return False
        elif client.get_rating() < 6:
            print("У вас слишком плохая кредитная история. В возможности получить кредит отказанно.")
            return False
        else:
            print("Все данные корректные! Поздравляем и переходим к следующему этапу!")
            return True
